...
$(function () {
    var taste = $(".cake-label .cake-taste-label .active").attr("data-taste");
    var spec = $(".cake-label .cake-spec-label .active").attr("data-spec");

    function fetchDataAndUpdatePage() {
        $.ajax({
            type: "get",
            url: `http://localhost:8080/products/cake?taste=${taste}&spec=${spec}`,
            dataType: "json",
            success: function (res) {
                if (res.code === 200) {
                    renderProductList(res.products);
                } else {
                    renderNoContent();
                }
            }
    ...
    function renderNoContent() {
        var html = `<section class="no-content">
                        <img src="img/404/no_content.png" alt="">
                        <p>抱歉，未找到相关商品</p>
                    </section>`;
        $(".cake-list .product-list").html(html);
